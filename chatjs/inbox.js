...
function createChatBox(chatboxtitle,toid,img,status,minimizeChatBox) {
    ...
    $(" <div />" ).attr("id","chatbox_"+chatboxtitle)
        ...
        $("#chatFrom").html('<div class="send-chat-box"><form id="f" action="post.php"><div class="col-sm-12"><textarea class="form-control chatboxtextarea" name="chattxt" onkeydown="javascript:return checkChatBoxInputKey(event,this,\''+chatboxtitle+'\',\''+toid+'\',\''+img+'\');" placeholder="Type your message"></textarea><div class="custom-send"><a onclick="javascript:chatemoji()" href="javascript:void(0)" id="toggle-emoji" class="cst-icon" data-toggle="tooltip" title="Insert Emojis"><i class="ti-face-smile"></i></a><a href="javacript:void(0)" class="cst-icon" data-toggle="tooltip" title="File Attachment"><i class="fa fa-paperclip"></i></a><div class="write-link attach"><input id="imageInput" type="file" name="file" onChange="uploadimage(\''+chatboxtitle+'\');"/></div><button onclick="javascript:return clickTosendMessage(\''+chatboxtitle+'\',\''+toid+'\',\''+img+'\');" class="btn btn-success btn-rounded" type="button" style="padding: 6px 10px;"><i class="fa fa-paper-plane-o"></i></button></div></div></form></div>');
			...
			if (item)	{ // fix strange ie bug
				...
				if (item.s == 2) {
                    ...
                    if (msgtype=="text"){
                        ...
                        message_content = Emotions (message_content);   // Set imotions
                    }

                    if (item.u == 2) {
                        $("#chatbox_"+chatboxtitle).prepend('<div class="col-xs-12 p-b-10 odd"><div class="chat-image"> <img alt="male" src="'+siteurl+'storage/user_image/'+img+'" class="bg-theme"> </div><div class="chat-body"><div class="chat-text"><h4>'+item.sender+'</h4><p>'+message_content+'</p><b>'+item.time+'</b> </div></div></div>');
					} else {
                        $("#chatbox_"+chatboxtitle).prepend('<div class="col-xs-12 p-b-10"><div class="chat-image"> <img alt="male" src="'+siteurl+'storage/user_image/'+img+'"> </div><div class="chat-body"><div class="chat-text"><h4>'+item.sender+'</h4><p>'+message_content+'</p><b>'+item.time+'</b> </div></div></div>');
					}
				}
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle,toid,img,send) {
    ...
    if((event.keyCode == 13 && event.shiftKey == 0) || (send == 1))  {
        ...
        $(chatboxtextarea).val('');
        $(chatboxtextarea).focus();
        $(chatboxtextarea).css('height','80px');
        if (message != '') {
            $.post(siteurl+"chat.php?action=sendchat", {to: chatboxtitle, toid: toid, message: message} , function(data){
                ...
                $("#chatbox_"+chatboxtitle).append('<div class="col-xs-12 p-b-10"><div class="chat-image"> <img alt="male" src="'+siteurl+'storage/user_image/'+img+'"> </div><div class="chat-body"><div class="chat-text"><h4>'+username+'</h4><p>'+message+'</p><b>10.00 am</b> </div></div></div>');
		...
		$.each(data.items, function(i,item){
			if (item)	{ // fix strange ie bug
                ...
                if (msgtype=="text")
                    ...
                    if (file_content.file_type == "image")
                        message_content = "<a url='"+file_content.file_path+"' onclick='trigq(this)'><img src='"+siteurl+"storage/user_files/small"+file_content.file_name+"' style='max-width:250px;min-height:100px;max-height: 150px;cursor: pointer;'/></a>";
                    else
                        message_content = "<a href='"+file_content.file_path+"'>Download : "+file_content.file_name+"</a>";
				...
				if (item.s == 2) {
					...
					$("#chatbox_"+chatboxtitle).append('<div class="col-xs-12 p-b-10 odd"><div class="chat-image"><img alt="male" src="'+siteurl+'storage/user_image/'+item.p+'" class="bg-theme"> </div><div class="chat-body"><div class="chat-text"><h4>'+chatboxtitle+'</h4><p>'+message_content+'</p><b>'+item.time+'</b> </div></div></div>');
					audiomp3.play();
					audioogg.play();
				}
